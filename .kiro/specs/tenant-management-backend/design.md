...
#### CQRS（Command Query Responsibility Segregation）の不採用

本設計では、CQRS（Command Query Responsibility Segregation）パターンを採用しません。以下の理由により、このドメインにはCQRSの導入が過剰であると判断しました：

**不採用の理由:**

1. **ドメインの複雑度が低い**
   - エンティティ数が少ない（Tenant、TenantSetting、AuditLog、ApplicationUser）
   - 集約がシンプル（Tenant集約のみ）
   - リレーションが単純（1対多が中心）

2. **読み取りと書き込みのスケーリング要件の差が小さい**
   - テナント識別は高頻度だが、キャッシュで対応可能
   - 書き込み操作は低頻度（テナント作成、更新、無効化）
   - 10,000テナント程度のスケールは標準的なアーキテクチャで十分

3. **読み取りモデルと書き込みモデルの分離が不要**
   - 読み取りと書き込みで同じエンティティ構造を使用
   - 複雑なクエリ要件がない（ページネーション、検索、フィルタリング程度）
   - 標準的なリポジトリパターンで十分対応可能
...
5. **複雑性の増加に見合う利益がない**
   - Command/Queryハンドラーの追加による実装コード量の増加（2-3倍）
   - メッセージバス/イベントストアの導入による運用コストの増加
   - 読み取りモデルと書き込みモデルの同期管理の複雑化

**採用するアーキテクチャ:**
- リポジトリパターン + アプリケーションサービス
- DDDの戦術パターン（集約、値オブジェクト、ドメインイベント）
- 必要に応じてキャッシュ層を追加
- 読み取り専用のクエリメソッドを最適化
